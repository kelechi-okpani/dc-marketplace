...
import { allAvailableBrands, categoryBrands, mainCategories, mockProducts } from '../utils/data/mockProducts';
...
export default function ProductCatalogPage() {
  ...
  const brands = activeCategory === 'all' 
  ? allAvailableBrands 
  : (categoryBrands[activeCategory] || []).sort();
  ...
  setSelectedBrands((prev) => {
    ...
    const exists = prev.some((b) => b.toLowerCase() === brandLower);
    
    if (exists) {
      // Remove it
      return prev.filter((b) => b.toLowerCase() !== brandLower);
    } else {
      // Add it
      return [...prev, brand];
    }
          ...
          <aside className={`bg-white rounded-xl border border-slate-200 p-4 shadow-xs space-y-5 md:block ${showMobileFilters ? 'block' : 'hidden'}`}>
            ...
            <div className="space-y-2 pt-3 border-t border-slate-100">
              ...
              <div className="max-h-36 overflow-y-auto no-scrollbar space-y-1.5 pr-1">
                {brands.map((brand) => (
                  <label key={brand} className="flex items-center gap-2.5 cursor-pointer group pt-2">
                    <input 
                      type="checkbox"
