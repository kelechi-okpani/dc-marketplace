...
export default function ServiceDetails() {
  ...
  // Retrieve matching profile dataset
  ...
  if (!baseProfile) {
    return (
      <div className="text-center py-20 bg-slate-50 min-h-screen">
        ...
        <button onClick={() => router.push('/')} className="mt-4 text-xs font-black text-blue-600 uppercase tracking-wider">Return to Marketplace</button>
      </div>
    );
  }
      ...
      <div className="border-b border-slate-100 bg-white sticky top-0 z-40 shadow-xs">
        <div className="mx-auto max-w-7xl px-4 h-14 flex items-center justify-between sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[11px] font-bold ${baseProfile.isAvailable ? 'bg-green-50 text-green-700' : 'bg-slate-100 text-slate-500'}`}>
              <span className={`h-1.5 w-1.5 rounded-full ${baseProfile.isAvailable ? 'bg-green-500' : 'bg-slate-400'}`} />
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-12 items-start">
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            {/* Primary Profile Spotlight Card Section */}
            <div className="flex flex-col sm:flex-row gap-6 items-start">
              <div className="w-full sm:w-48 aspect-square rounded-2xl bg-slate-100 overflow-hidden border border-slate-200 shadow-xs relative shrink-0">
                {/* Fallback avatar block matching design color profiles */}
              ...
              <div className="space-y-3 flex-1">
                <div className="space-y-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="inline-flex items-center gap-1 rounded-md bg-blue-600 px-2 py-0.5 text-[10px] font-black text-white uppercase tracking-wider shadow-xs">
            ...
            {/* Recent Work / Project Portfolio Grid Block */}
            <div className="space-y-4 pt-4 border-t border-slate-100">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {extendedDetails.projects.map((project, idx) => (
                  <div key={idx} className="group relative rounded-xl border border-slate-200 bg-white overflow-hidden shadow-xs hover:border-blue-300 transition-all">
                    ...
                    <div className="p-3 bg-white border-t border-slate-100">
                      <span className="text-xs font-bold text-slate-800 group-hover:text-blue-600 transition-colors">{project.title}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {/* ================= RIGHT SIDEBAR METRICS TRAIL ================= */}
          <div className="lg:col-span-4 space-y-4 lg:sticky lg:top-20">
            
            {/* Rates & Direct Action Checkout Card Container */}
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs space-y-4">
              ...
              <div>
                ...
                <p className="text-[11px] font-medium text-slate-400 mt-0.5">or Fixed milestone-based project structures</p>
              </div>

              <div className="space-y-2 pt-2">
                <button className="w-full inline-flex items-center justify-center rounded-xl bg-blue-600 py-3 text-xs font-black uppercase tracking-wider text-white shadow-md shadow-blue-100 hover:bg-blue-700 transition-colors cursor-pointer">
            ...
            {/* Skills & Verified Endorsements Card Container */}
            ...
            {/* Verification & License Registry Credentials Card Container */}
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs space-y-3.5">
              <div className="flex items-center gap-1.5 text-slate-800">
