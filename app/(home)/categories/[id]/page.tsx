  ...
  return (
    <div className="bg-[#f1f5f9] min-h-screen pb-16 font-sans text-slate-800 antialiased">
      ...
      <div className="mx-auto max-w-[1400px] px-4 pt-4 mb-2">
        <Link 
          href="/"
          className="inline-flex items-center gap-2 text-xs font-bold text-slate-500 hover:text-green-600 transition-colors"
        >
          ...
          <div className="flex items-center gap-3 w-full md:max-w-xl">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 text-xs font-medium rounded-lg border border-slate-200 bg-slate-50 focus:bg-white focus:border-green-600 focus:outline-none transition-all text-slate-900"
              />
            </div>
          </div>
          ...
          <aside className={`bg-white rounded-xl border border-slate-200 p-4 shadow-sm space-y-5 md:block ${showMobileFilters ? 'block' : 'hidden'}`}>
            <div className="flex items-center justify-between border-b border-slate-100 pb-2.5">
              <div className="space-y-0.5">
                <span className="text-[10px] font-black tracking-widest text-green-600 uppercase">Viewing Directory</span>
                <h3 className="text-xs font-black text-slate-900 uppercase truncate max-w-[180px]">{currentCategoryObj.label}</h3>
              </div>
              <button 
                type="button"
                onClick={resetAllFilters}
                className="text-[11px] font-bold text-slate-400 hover:text-green-600 flex items-center gap-1 cursor-pointer"
              >
            ...
            <div className="space-y-1">
              ...
              <button
                ...
                className={`w-full text-left px-2.5 py-2 rounded-lg text-xs font-bold transition-all flex items-center justify-between cursor-pointer ${
                  activeSubCategory === 'all' ? 'bg-green-600 text-white' : 'text-slate-600 hover:bg-slate-50'
                }`}
              >
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left px-2.5 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center justify-between cursor-pointer ${
                      activeSubCategory === sub ? 'bg-green-50 text-green-600' : 'text-slate-600 hover:bg-slate-50'
                    }`}
                  >
                    <span>{sub}</span>
                    {subCount > 0 && (
                      <span className={`text-[10px] font-mono font-bold px-1.5 rounded ${activeSubCategory === sub ? 'bg-green-200 text-green-700' : 'bg-slate-100 text-slate-400'}`}>
                        {subCount}
                      </span>
                    )}
                  </button>
              ...
              <div className="pt-3 border-t border-slate-100">
                <label className="flex items-center gap-2.5 cursor-pointer select-none">
                  <input 
                    ...
                    className="rounded border-slate-300 text-green-600 h-4 w-4 focus:ring-green-500"
                  />
                  ...
                  {availableBrandsForCategory.map((brand) => (
                    <label key={brand} className="flex items-center gap-2.5 cursor-pointer group select-none mt-3">
                      <input 
                        ...
                        className="rounded border-slate-300 text-green-600 h-3.5 w-3.5 focus:ring-green-500"
                      />
            ...
            <div className="space-y-2 pt-3 border-t border-slate-100">
              ...
              <div className="grid grid-cols-2 gap-2">
                <input
                  ...
                  className="w-full p-2 text-xs font-bold rounded-lg bg-slate-50 border border-slate-200 focus:outline-none focus:bg-white focus:border-green-600 transition-all"
                />
                <input
                  type="number"
                  placeholder="Max"
          ...
          <section className="md:col-span-3 space-y-4">
            <div className="bg-white rounded-xl border border-slate-200 px-4 py-2.5 flex items-center justify-between shadow-sm">
              <p className="text-xs font-medium text-slate-500">
                Showing <span className="font-bold text-slate-800">{recordMetrics.start}-{recordMetrics.end}</span> of <span className="font-black text-green-600">{recordMetrics.total}</span> matches inside <span className="font-bold text-slate-800">"{currentCategoryObj.label}"</span>
              </p>
            </div>
